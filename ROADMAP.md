...
## Path to Expert Read-Only Query Mode

Goal: let expert users write controlled read-only SQL queries as an alternative
to building filters one by one.

This feature should live beside the existing guided filters, not replace them.
The Dataset workspace should expose a tabbed panel:

```text
[ Filters ] [ Query ]
```

The `Filters` tab keeps the current guided filter builder. The `Query` tab adds
a read-only SQL surface for advanced users.
...
- [x] Dataset BLoC state fields:
      ...
      - query text
      - query loading state
      - query validation/runtime error
      - query result rows/columns if they differ from the active sheet schema
