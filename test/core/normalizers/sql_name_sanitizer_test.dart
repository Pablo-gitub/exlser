  ...
  group('SqlNameSanitizer', () {
    group('sanitizeColumnName', () {
      test('should convert to lowercase and trim spaces', () {
        final result = SqlNameSanitizer.sanitizeColumnName('  My Column Name  ');
      ...
      test('should replace non-alphanumeric characters with underscores', () {
        ...
        expect(result1, 'price'); // Notice trailing/leading underscores are removed
        expect(result2, 'user_email');
      });

      test('should handle consecutive special characters with a single underscore', () {
      ...
      test('should remove leading and trailing underscores', () {
        final result = SqlNameSanitizer.sanitizeColumnName('___hidden_field___');
        expect(result, 'hidden_field');
      });

      test('should use fallback prefix if name becomes empty after sanitization', () {
        expect(SqlNameSanitizer.sanitizeColumnName(''), 'col');
        expect(SqlNameSanitizer.sanitizeColumnName('   '), 'col');
        expect(SqlNameSanitizer.sanitizeColumnName('🚀🚀'), 'col'); // Emojis only
        expect(SqlNameSanitizer.sanitizeColumnName('!@#\$'), 'col'); // Special chars only
      });
  
      test('should add prefix if name starts with a number', () {
        expect(SqlNameSanitizer.sanitizeColumnName('1st_place'), 'col_1st_place');
        expect(SqlNameSanitizer.sanitizeColumnName('2023_Sales'), 'col_2023_sales');
      });
      ...
      test('should avoid duplicates by appending a counter', () {
        ...
        // Base collision
        expect(
          SqlNameSanitizer.sanitizeColumnName('My Column', existingNames: existingNames),
          'my_column_2',
        );

        // Keyword collision
        expect(
          SqlNameSanitizer.sanitizeColumnName('select', existingNames: existingNames),
          'select_col_1',
        );
      });
      ...
      test('should handle completely chaotic inputs resolving to unique names', () {
        ...
        expect(
          SqlNameSanitizer.sanitizeColumnName('🎉', existingNames: existingNames),
          'col_2',
        );
      });
    });
    ...
    group('sanitizeTableName', () {
      ...
      test('should prepend "tbl" if name starts with a number', () {
        expect(SqlNameSanitizer.sanitizeTableName('2023_Data'), 'tbl_2023_data');
      });
